...
from transformer.Layers import FFTBlock, DecBlock, PreNet, PostNet, Linear
...
class newDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer_stack = nn.ModuleList([DecBlock(d_model,
                                                   d_inner,
                                                   n_head,
                                                   d_k, d_v,
                                                   dropout=dropout)
    ...
    def forward(self, dec_input, enc_output, enc_pos, dec_pos, return_attns=False):

        dec_slf_attn_list = []

        # -- Prepare masks
        slf_attn_mask = get_attn_key_pad_mask(seq_k=enc_pos, seq_q=dec_pos)
        non_pad_mask = get_non_pad_mask(dec_pos)
