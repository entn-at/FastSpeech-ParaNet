...
from Networks import get_position
...
def main(args):
    ...
    for epoch in range(hp.epochs):
        for i, data_of_batch in enumerate(training_loader):
            ...
            mel_output, mel_output_postnet = model(
                src_seq, src_pos, dec_pos=dec_pos)

            # print(mel_output.size())
            # print(mel_tgt.size())
            # print()
            mel_len_list.append(mel_output.size(1))
            mel_tgt_list.append(mel_tgt.size(1))
            if current_step == 500:
                break
            ...
            mel_loss, mel_postnet_loss = fastspeech_loss(
                mel_output, mel_output_postnet, mel_tgt)
    ...
    plt.figure()
    plt.plot([i for i in range(len(mel_len_list))],
             mel_len_list, color="r", lw=0.3)
    plt.plot([i for i in range(len(mel_tgt_list))],
             mel_tgt_list, color="b", lw=0.3)
